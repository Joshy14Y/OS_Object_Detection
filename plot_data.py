...
def plot_general_stats(classes, counts, mean_confidences, path):
    ...
    directory = f"detections/{title.split('_')[0]}/{date}/stats/"
    ...
    plt.tight_layout()
    plt.savefig(f"detections/{title.split('_')[0]}/{date}/stats/{title}", bbox_inches='tight')
...
def plot_class_and_general_stats(json_data):
    ...
    for video, frames in json_data.items():
        ...
        class_mean_confidence = {class_name: np.mean(confidences) for class_name, confidences in
    ...
    all_class_mean_confidence = {class_name: np.mean(confidences) for class_name, confidences in
                                 all_class_confidences.items()}
    ...
    plot_general_stats(classes, counts, mean_confidences, f"{fp}_General")
